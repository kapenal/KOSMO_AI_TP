...
# 모델 및 데이터 불러오기
model = joblib.load(os.path.join(PATH, "word2vec_movie.model")) # Word2Vec 모델 -> 단어 임베딩(벡터)
movie_vectors = joblib.load(os.path.join(PATH, "movie_vectors.pkl")) # 영화별 임베딩 -> 벡터 유사도 기반 추천
movies = joblib.load(os.path.join(PATH, "movies.pkl")) # 영화 제목/정보 -> 결과 표시용
title_tokens = joblib.load(os.path.join(PATH, "title_tokens.pkl")) # 영화 제목 토큰화 결과 -> 입력 유사 제목 찾기
raw_word_scores = joblib.load(os.path.join(PATH, "soynlp_word_scores.pkl")) # 단어 점수 정보 -> 전처리 단어 필터링
...
# 가장 유사한 영화 제목 인덱스 찾기 -> title_tokens.pkl과 자카드 유사도를 이용
...
def recommend_movie(input_title, movies, movie_vectors, title_tokens, top_n=5):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        cosine_similarities = np.nan_to_num(cosine_similarities) # 코사인 유사도
    ...
    for i in top_indices:
        ...
        genre_overlap = jaccard_similarity(input_genres, rec_genres) # 장르는 자카드 유사도
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(json.dumps({"error": "영화 제목을 입력하세요."}, ensure_ascii=False))
        sys.exit(1)
